    ...
    var handledzNumber = function () {
        if ($('.dz-number').length > 0) {
            $('.dz-number').on('input', function () {
                ...
                if (numericVal.length > 13) {
                    $(this).val(numericVal.slice(0, 13));
                } else {
                    $(this).val(numericVal);
                }
            });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Ghi đè xử lý form mặc định
    $(".dzForm").submit(function(e) {
        e.preventDefault();
        console.log("Form submitted"); // Debug log
        $('.dzFormMsg').html('<div class="gen alert alert-info">Đang gửi...</div>');
        var dzFormAction = $(this).attr('action');
        var dzFormData = $(this).serialize();
        
        console.log("Form action:", dzFormAction); // Debug log
        console.log("Form data:", dzFormData); // Debug log
        ...
        $.ajax({
            ...
            success: function(dzRes) {
                console.log("Response:", dzRes); // Debug log
                ...
                if(dzRes.status == 1) {
                    ...
                    // Lưu template hiện tại trước khi reset form
                    var currentTemplate = '';
                    var selectedOption = document.querySelector('#contact-type option:checked');
                    if (selectedOption) {
                        currentTemplate = selectedOption.getAttribute('data-template-name');
                    }
                    
                    $('.dzForm')[0].reset();
                    $('.dzFormMsg').html('<div class="gen alert alert-success">'+dzRes.msg+'</div>');
                    
                    // Ẩn các template box
                    $('.template-box').hide();
                    
                    // Khôi phục template đã chọn
                    if (currentTemplate) {
                        // Chọn lại option trong dropdown
                        var contactType = document.getElementById('contact-type');
                        for (var i = 0; i < contactType.options.length; i++) {
                            if (contactType.options[i].getAttribute('data-template-name') === currentTemplate) {
                                contactType.selectedIndex = i;
                                break;
                            }
                        }
                        
                        // Hiển thị lại template tương ứng
                        var templateToShow = document.querySelector('.template-box[data-template-name="' + currentTemplate + '"]');
                        if (templateToShow) {
                            templateToShow.style.display = 'flex';
                        }
                    }
                } else if(dzRes.error && dzRes.message) {
                    // Xử lý lỗi từ API với format mới
                    var errorMsg = '';
                    if(typeof dzRes.message === 'object' && dzRes.message.message) {
                        errorMsg = dzRes.message.message;
                    } else {
                        errorMsg = dzRes.message;
                    }
                    if (typeof toastr !== 'undefined') {
                        toastr.error(errorMsg);
                    }
                    $('.dzFormMsg').html('<div class="err alert alert-danger">'+errorMsg+'</div>');
                } else {
                    // Lỗi thông thường
                    var errorMsg = dzRes.msg || "Có lỗi xảy ra";
                    if (typeof toastr !== 'undefined') {
                        toastr.error(errorMsg);
                    }
                    $('.dzFormMsg').html('<div class="err alert alert-danger">'+errorMsg+'</div>');
            ...
            error: function(xhr, status, error) {
                ...
                if(xhr.responseJSON) {
                    console.log("Response JSON:", xhr.responseJSON); // Debug log
                    
                    if(xhr.responseJSON.message && typeof xhr.responseJSON.message === 'object') {
                        // Xử lý lỗi từ Laravel validation
                        errorMessage = xhr.responseJSON.message.message || "Dữ liệu không hợp lệ";
                ...
                $('.dzFormMsg').html('<div class="err alert alert-danger">'+errorMessage+'</div>');
            },
            complete: function() {
