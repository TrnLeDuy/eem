<template>
    <div class="div">
                        ...
                        <div class="sc-table">
                            <div class="tool-bar el-row" style="padding-bottom: 20px">
                                <div class="actions el-col el-col-8">
                                    <router-link :to="{ name: 'admin.project.category.create' }">
                            ...
                            <el-table ref="pageTable" v-loading.body="tableIsLoading" :data="data" stripe
                                ...
                                <el-table-column :label="trans('categories.table.title')" prop="translations.title">
                                    <template slot-scope="scope">
                                        <a :href="editRoute(scope.row.id)" @click.prevent="goToEdit(scope.row.id)">
                                            {{ scope.row.translations.title }}
                                        </a>
                                    </template>
                                </el-table-column>
                                <el-table-column :label="trans('categories.table.slug')" prop="translations.slug">
                                    <template slot-scope="scope">
                                        <a :href="editRoute(scope.row.id)" @click.prevent="goToEdit(scope.row.id)">
        ...
        <button v-show="false" v-shortkey="['c']"
            @shortkey="pushRoute({ name: 'admin.project.category.create' })"></button>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            queryServer(customProperties) {
                ...
                axios
                    .get(
                        route("api.project.category.indexServerSide", {
                            ...properties,
                            ...customProperties,
                        })
                    )
