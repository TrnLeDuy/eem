...
<div class="location-groups">
	...
	<div data-id="{{ $group_id }}" class="location-group">
		...
		<h4>{{ trans('dynamicfield::location.show_if') }}</h4>
		<?php else: ?>
		<h4>{{ trans('dynamicfield::location.or') }}</h4>
		<?php endif; ?>
			...
			<tbody>
				...
				<tr data-id="{{ $rule_id }}">
					<td class="param">
						...
						{!! Form::select($parameter_name, $arrParameter,@$parameter_selected, ['class' => "form-control
						drop-parameter","onChange"=>"changeLocationParameter(this,'$value_selected')"]) !!}
					</td>
					<td class="operator">
						<?php 
	...
	<h4>{{ trans('dynamicfield::location.or') }}</h4>
	<a onclick="location_add_group(this,{{ $group_index_id }})" class="button location-add-group">{{ trans('dynamicfield::location.add_rule_group') }}</a>
