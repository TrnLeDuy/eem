...
class GroupFieldsController extends AdminBaseController
{
    ...
    public function store(GroupFieldRequest $request)
    {
        ...
        return redirect()->route('admin.dynamicfield.group.index')->withSuccess(trans('dynamicfield::dynamicfield.title.field_group'));
    }
    ...
    {
        // Sắp xếp các trường theo thứ tự số trong tiêu đề
        $fields = $group->fields()->get()->sort(function ($a, $b) {
            // Trích xuất số từ tiêu đề (ví dụ: "01. abc" -> 1)
            preg_match('/^(\d+)\./', $a->title, $matchesA);
            preg_match('/^(\d+)\./', $b->title, $matchesB);
            
            $numA = isset($matchesA[1]) ? (int)$matchesA[1] : 999;
            $numB = isset($matchesB[1]) ? (int)$matchesB[1] : 999;
            
            return $numA - $numB;
        });
        
        // Gán lại các trường đã sắp xếp
        $group->setRelation('fields', $fields);
