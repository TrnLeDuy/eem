<div class="box-body">
    <div class="box-body">
        <div class='form-group{{ $errors->has("{$lang}.title") ? ' has-error' : '' }}'>
            {!! Form::label("{$lang}[title]", trans('page::posts.form.title')) !!}
            {!! Form::text("{$lang}[title]", old("{$lang}.title"), ['class' => 'form-control', 'data-slug' => 'source',
            'placeholder' => trans('page::posts.form.title')]) !!}
            {!! $errors->first("{$lang}.title", '<span class="help-block">:message</span>') !!}
        </div>
        <div class='form-group{{ $errors->has("{$lang}.slug") ? ' has-error' : '' }}'>
            {!! Form::label("{$lang}[slug]", trans('page::posts.form.slug')) !!}
            {!! Form::text("{$lang}[slug]", old("{$lang}.slug"), ['class' => 'form-control slug', 'data-slug' =>
            'target', 'placeholder' => trans('page::posts.form.slug')]) !!}
            {!! $errors->first("{$lang}.slug", '<span class="help-block">:message</span>') !!}
        </div>
        <div class='form-group{{ $errors->has("{$lang}.author") ? ' has-error' : '' }}'>
            {!! Form::label("{$lang}[author]", trans('page::posts.form.author')) !!}
            {!! Form::text("{$lang}[author]", old("{$lang}.author"), ['class' => 'form-control', 'placeholder' => trans('page::posts.form.author')]) !!}
        ...
        <div class='form-group{{ $errors->has("{$lang}.sumary") ? ' has-error' : '' }}'>
            ...
            <textarea class="form-control" name="{{$lang}}[sumary]" rows="5"
                            cols="80">{{ old("$lang.sumary") }}</textarea>
            {!! $errors->first("{$lang}.sumary", '<span class="help-block">:message</span>') !!}
        </div>

        @editor('body', trans('page::posts.form.body'), old("{$lang}.body"), $lang)
