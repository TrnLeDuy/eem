<div class="box-body">
    <div class="box-body">
        <div class='form-group{{ $errors->has("{$lang}.title") ? ' has-error' : '' }}'>
            ...
            {!! Form::text("{$lang}[title]", old("{$lang}.title", $old), ['class' => 'form-control', 'data-slug' =>
            'source', 'placeholder' => trans('page::pages.form.title')]) !!}
        ...
        <div class='form-group{{ $errors->has("{$lang}[slug]") ? ' has-error' : '' }}'>
            ...
            {!! Form::text("{$lang}[slug]", old("{$lang}.slug", $old), ['class' => 'form-control slug', 'data-slug' =>
            'target', 'placeholder' => trans('page::pages.form.slug')]) !!}
            {!! $errors->first("{$lang}.slug", '<span class="help-block">:message</span>') !!}
        </div>
        <div class='form-group{{ $errors->has("{$lang}.author") ? ' has-error' : '' }}'>
            {!! Form::label("{$lang}[author]", trans('page::posts.form.author')) !!}
            <?php $old = $post->hasTranslation($lang) ? $post->translate($lang)->author : '' ?>
            {!! Form::text("{$lang}[author]", old("{$lang}.author", $old), ['class' => 'form-control', 'placeholder' => trans('page::posts.form.author')]) !!}
            {!! $errors->first("{$lang}.author", '<span class="help-block">:message</span>') !!}
        </div>
        <div class='form-group{{ $errors->has("{$lang}.sumary") ? ' has-error' : '' }}'>
            {!! Form::label("{$lang}[sumary]", trans('page::posts.form.sumary')) !!}
            <?php $old =$post->hasTranslation($lang) ?$post->translate($lang)->sumary : '' ?>
            <textarea class="form-control" name="{{$lang}}[sumary]" rows="5"
                            cols="80">{{ old("$lang.sumary", $old) }}</textarea>
            {!! $errors->first("{$lang}.sumary", '<span class="help-block">:message</span>') !!}
        </div>


        <?php $old =$post->hasTranslation($lang) ?$post->translate($lang)->body : '' ?>
        @editor('body', trans('page::pages.form.body'), old("$lang.body", $old), $lang)
        @if(config('asgard.page.config.partials.translatable.edit_post') !== [])
      
        @foreach (config('asgard.page.config.partials.translatable.edit_post') as $partial)
            ...
            <div style="height: 0px;" id="collapseTwo-{{$lang}}" class="panel-collapse collapse">
                <div class="box-body">
                    <div class='form-group{{ $errors->has("{$lang}[meta_title]") ? ' has-error' : '' }}'>
                        ...
                        {!! Form::text("{$lang}[meta_title]", old("{$lang}.meta_title", $old), ['class' =>
                        "form-control", 'placeholder' => trans('page::pages.form.meta_title')]) !!}
            ...
            <div style="height: 0px;" id="collapseFacebook-{{$lang}}" class="panel-collapse collapse">
                <div class="box-body">
                    <div class='form-group{{ $errors->has("{$lang}[og_title]") ? ' has-error' : '' }}'>
                        ...
                        <?php $old =$post->hasTranslation($lang) ?$post->translate($lang)->og_title : '' ?>
                        {!! Form::text("{$lang}[og_title]", old("{$lang}.og_title", $old), ['class' => "form-control",
                        'placeholder' => trans('page::pages.form.og_title')]) !!}
                    ...
                    <div class='form-group{{ $errors->has("{$lang}[og_description]") ? ' has-error' : '' }}'>
                        ...
                        <?php $old =$post->hasTranslation($lang) ?$post->translate($lang)->og_description : '' ?>
                        <textarea class="form-control" name="{{$lang}}[og_description]" rows="10"
                            cols="80">{{ old("$lang.og_description", $old) }}</textarea>
                    ...
                    <div class="form-group{{ $errors->has(" {$lang}[og_type]") ? ' has-error' : '' }}">
                        ...
                        <?php $oldType =$post->hasTranslation($lang) ?$post->translate($lang)->og_type : '' ?>
