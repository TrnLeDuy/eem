...
function addField(o) {
	...
	// Tự động sắp xếp sau khi thêm trường mới
	setTimeout(function() {
		// Kích hoạt sự kiện sắp xếp
		$list.find('.sort-by-title').trigger('click');
	}, 100);
}
function addRepeaterField(Id) {
	...
	$_new.find('[name*="[0]"]').each(function () {
		var id = $(this).attr('id');
		var name = $(this).attr('name');
		$(this).attr('id', id.replace("_0_", '_' + new_id + '_'));
		$(this).attr('name', name.replace("[0]", '['+new_id+']'));
...
// Add function to save field order
function saveFieldOrder($container) {
	if (!$container.hasClass('sortable')) {
		$container = $container.find('.sortable');
	}
	
	var fields = [];
	$container.find('.another-field').each(function(index) {
		fields.push({
			id: $(this).data('id'),
			order: index + 1
		});
...
// Add this function to sort fields by their numeric prefix
function sortFieldsByNumericPrefix() {
	$('.sortable').each(function() {
		var $container = $(this);
		var $items = $container.children('.another-field').get();
		
		$items.sort(function(a, b) {
			var titleA = $(a).find('td.field-label a').first().text() || '';
			var titleB = $(b).find('td.field-label a').first().text() || '';
			
			// Extract numeric prefix if exists (e.g., "01. abc" -> 1)
			var numA = parseInt(titleA.match(/^(\d+)\.\s*/) ? titleA.match(/^(\d+)\.\s*/)[1] : 0);
			var numB = parseInt(titleB.match(/^(\d+)\.\s*/) ? titleB.match(/^(\d+)\.\s*/)[1] : 0);
...
$(document).ready(function() {
	$('.field_infor').each(function() {
		var $panel = $(this);
		var $button = $('<button class="btn btn-sm btn-default sort-by-title">Sort by Title Number</button>');
		$panel.find('.add-field-button').after($button);
		
		$button.on('click', function(e) {
			e.preventDefault();
			sortFieldsByNumericPrefix();
		});
	});
	
	// Optional: Auto-sort on page load
	// Uncomment this if you want fields to be automatically sorted when the page loads
	// setTimeout(function() {
	//     sortFieldsByNumericPrefix();
	// }, 500);
});

// Thêm đoạn code này để xử lý sự kiện submit form
$(document).ready(function() {
    // Lưu thứ tự hiện tại vào hidden field trước khi submit form
    $('form').on('submit', function() {
        ...
        // Tạo hoặc cập nhật hidden field để lưu thứ tự
        var $orderInput = $('input[name="field_order"]');
        if ($orderInput.length === 0) {
            $orderInput = $('<input type="hidden" name="field_order">');
            $(this).append($orderInput);
        }
        $orderInput.val(JSON.stringify(orderData));
    });
});
